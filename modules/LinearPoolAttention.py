...
class LowRank(nn.Module):
    def __init__(self, embed_dim, att_type, att_heads, att_mid_dim, att_mid_drop=0.1):
        ...
        self.linera=nn.Linear(embed_dim, output_dim)
        #act = utils.activation(ACT)
        act = activation(ACT)
        if act is not None:
    ...
    def forward1(self, query, key, mask, value1, value2, precompute=False):
        batch_size = query.size()[0]
        lin = self.linera(query)
    ...
    def forward(self, query, key, mask, value1, value2, precompute=False):
        ...
        value1 = value1.permute(1, 0, 2).contiguous()
        value2 = value2.permute(1, 0, 2).contiguous()
        batch_size = query.size()[0]
        ...
        attn,alpha_spatial = self.attn_net.forward(attn_map, mask, v1, v2)
