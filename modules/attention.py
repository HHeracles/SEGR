...
class PositionAttentionKeyAndValue(nn.Module):
    def __init__(self, max_length, in_channels=512, num_channels=64, 
                 h=8, w=32, mode='nearest', **kwargs):
        super().__init__()
        self.max_length = max_length
    ...
    def forward(self, x):
        ...
        features = []
        for i in range(0, len(self.v_encoder)):
            v = self.v_encoder[i](v)
            features.append(v)
        for i in range(0, len(self.v_decoder) - 1):
            v = self.v_decoder[i](v)
            v = v + features[len(self.v_decoder) - 2 - i]
        v = self.v_decoder[-1](v)
...
class PositionCodeAttention(nn.Module):
    def __init__(self, max_length, in_channels=512, num_channels=64, 
                 h=8, w=32, mode='nearest', **kwargs):
        super().__init__()
        self.max_length = max_length
        ...
        self.pos_encoder = PositionalEncoding(in_channels, dropout=0, max_len=max_length)
        self.project = nn.Linear(in_channels, in_channels)
        self.Linear = nn.Linear(in_channels, in_channels-w-h)
        
        self.x_onehot_size = int(w)
        self.y_onehot_size = int(h)
    ...
    def forward(self, x):
        x_t, y_t = np.meshgrid(np.linspace(0, self.x_onehot_size - 1, self.x_onehot_size), np.linspace(0, self.y_onehot_size - 1, self.y_onehot_size))
        ...
        x = torch.cat(
            [x, x_onehot_embedding, y_onehot_embedding], 1
        )

        N, E, H, W = x.size()
        k, v = x, x  # (N, E, H, W)=(B,C,H,W)
        ...
        attn_vecs = torch.bmm(attn_scores, v)  # (N, T, E)
        attn_vecs = self.Linear(attn_vecs)
