...
class IterationCallback(LearnerTensorboardWriter):
    ...
    def on_train_begin(self, n_epochs, **kwargs):
        ...
        self.best = -float('inf')
        begin_metrics = self._validate()
        self.best = begin_metrics[2]
        #print("n_epochs:",n_epochs)
        print("begin_metrics:",self.best)
        #input()
        self.timer.tic()
        #self.star_time = self.timer.start()
        if self.host:
    ...
    def on_batch_end(self, iteration, epoch, last_loss, smooth_loss, train, **kwargs):
        ...
        if iteration % self.eval_iters == 0  or epoch>=8:
            ...
            #self.best = 100
            # 3. Save best model
            #self.current_time = self.timer.stop()
...
class TextAccuracy(Callback):
    ...
    def __init__(self, charset_path, max_length, case_sensitive, model_eval):
        ...
        assert self.model_eval in ['vision', 'language', 'alignment','grahpyReason','voting']
    ...
    def _get_output(self, last_output):
        #print("self.model_eval:",self.model_eval)
        #print("len(last_output):",len(last_output))
        #input()
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        ...
        #print("callback_output['name']",output['name'])
        #print("logits_1:",logits)
        pt_text, pt_scores, pt_lengths_ = self.decode(logits)
        #print("logits.shape:",logits.shape)
        #print("len(pt_text):",len(pt_text))
    ...
    def on_batch_end(self, last_input, last_output, last_target, **kwargs):
        ...
        for gt, pt, image, attn, logit in zip(gt_text, pt_text, images, attn_scores, logits):
            ...
            if gt != pt:
                ...
                false_prediction.append(f'{self.c}_{gt}\t{pt}\n')
            ...
            if not self.image_only:
                ...
                #print("self.c:",self.c)
                #print("self.attn_root:",self.attn_root)
            self.c += 1
