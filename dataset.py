...
class ImageDataset(Dataset):
    ...
    def get(self, idx):
        with self.env.begin(write=False) as txn:
            ...
            #print("label_key.encode():",label_key.encode())
            ...
            try:
                ...
                return self._next_image(idx)
            '''
            saveImagePath = str(self.path)+'/tempfile/'+str(idx+1)+".jpg"
            #print("saveImagePath:",saveImagePath)
            
            image.save(saveImagePath)
            saveTextPath = str(self.path) + '/tempfile/'+str(idx+1)+".txt"
            file = open(saveTextPath,'w')
            file.write(label+'\n')
            file.close()
            '''
    ...
    def __getitem__(self, idx):
        #print("dataset_self.path:",self.path)
        #print("dataset_self.name:",self.name)
        #print("dataset_idx:",idx)
        image, text, idx_new = self.get(idx)
        #print("dataset_image:",image)
        #print("dataset_text:",text)
        #print("dataset_idx_new:",idx_new)
        #print("\n")
        #print("idx:",idx,"idx_new:",idx_new)
        ...
        #print("out_image.shape:",image.shape)
        #print("out_y:",y)
        return image, y
